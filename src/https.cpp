...
std::string read_string(std::string fn){
    std::ifstream inCert;
    inCert.open(fn, std::ios_base::in);
    std::stringstream ss_cert;
    ss_cert << inCert.rdbuf();
    std::string str = ss_cert.str();
    inCert.close();
    return str;
}
void load_certificate(net::ssl::context& ctx){
    ...
    cert = read_string("./rabbit.cd-lucian.com.pem");
    key = read_string("./rabbit.cd-lucian.com.key");
